...
(function() {
    ...
    /* =========================================================================
     * TRUSTEDHTML POLICY (CSP compliance)
     * ===================================================================== */

    const trustedPolicy = (typeof trustedTypes !== 'undefined' && trustedTypes.createPolicy)
        ? trustedTypes.createPolicy(`${CSS_PREFIX}-policy`, { createHTML: s => s })
        : { createHTML: s => s };
    ...
    function buildSettingsPanel() {
        ...
        headerLeft.append(logo, title, version);
        ...
        githubBtn.className = `${CSS_PREFIX}-btn ${CSS_PREFIX}-btn-secondary`;
        githubBtn.id = `${CSS_PREFIX}-github-btn`;
        githubBtn.textContent = 'GitHub';
        footer.append(statsEl, githubBtn);
        ...
        panel.querySelector(`#${CSS_PREFIX}-github-btn`).addEventListener('click', () => {
            window.open('https://github.com/SysAdminDoc/youtubeadblock', '_blank');
        });
    ...
    function buildContent() {
        ...
        function createFilterStat(label, value, id) {
            const stat = document.createElement('div');
            stat.className = `${CSS_PREFIX}-filter-stat`;
            const labelEl = document.createElement('span');
            labelEl.className = `${CSS_PREFIX}-filter-stat-label`;
            labelEl.textContent = label;
            const valueEl = document.createElement('span');
            valueEl.className = `${CSS_PREFIX}-filter-stat-value`;
            if (id) valueEl.id = id;
        ...
        filterInfo.append(
            createFilterStat('Filter Version', state.filters?.version || '?', `${CSS_PREFIX}-fv`),
            createFilterStat('Source', state.filterSource, `${CSS_PREFIX}-fs`),
            createFilterStat('Last Updated', state.lastFilterUpdate ? new Date(state.lastFilterUpdate).toLocaleString() : 'Never', `${CSS_PREFIX}-flu`),
            createFilterStat('Selectors', String((state.filters?.cosmeticSelectors?.length || 0) + (state.filters?.upsellSelectors?.length || 0)))
        );

...
- [ ] P2, Gate the build-freshness check on content, not the version string
  Why: the parity test compares `SCRIPT_VERSION` between the userscript and the generated bundle, so a stale `extension/main.js` at the same version passes. This matches a recorded failure where a chained build silently no-opped and the suite ran green against the previous build.
  Evidence: tests/repo-contract.test.mjs:316-353 (version comparison), :355-377 (six substring markers), and the working notes from 2026-08-14.
...
- [ ] P2, Verify the log_event and generate_204 block rules do not break playback state
  ...
  Evidence: extension/rules/network-rules-source.json rules 16 and 18, plus the project's 2026-08-13 network lesson.
  ...
  Acceptance: a dated check records whether watch history, resume position and playback progress survive with both rules enabled; any rule that breaks them is narrowed to ad-exclusive paths or removed, and the finding is recorded in the working notes.
  Complexity: S
...
- [ ] P3, Re-evaluate the player-request strategy ladder against the v0.4.1 regression
  ...
  Acceptance: the v0.4.1 breakage is first reproduced or shown not to reproduce on current YouTube, dated and recorded in the working notes. Only then can any adopted step ship behind a default-off toggle with automatic degradation on failure and per-attempt attribution. A fixture must assert that normal playback, comments and SPA navigation are unaffected. If the regression still reproduces, this item is closed as rejected rather than implemented.
  Complexity: M
